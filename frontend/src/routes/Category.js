...
export default function Category() {
  ...
  let _lessThenPrice;
  if (location.state) {
    if (location.state.hasOwnProperty('category')) {
      // Certain Cateogry from Home
      fromWhere = location.state.category;
    } else if (location.state.hasOwnProperty('homeSearch')) {
      // Search from Home
  ...
  const saveSearchList = (search) => {
    ...
    if (pastSearchList.includes(search)) {
      // If the search keyword already exist, delete it
      pastSearchList.splice(pastSearchList.indexOf(search), 1);
    }
    localStorage.setItem(
      'pastSearchList',
      JSON.stringify([search, ...pastSearchList])
  ...
  useEffect(() => {
    if (!_products && data[0]) {
      if (fromWhere) {
        if (
          ['MENS', 'WOMENS', 'FOOD', 'FUNITURE', 'DIGITAL'].includes(fromWhere)
        ) {
          setProducts(
            data.filter((arr) => {
          ...
          const searchData = data.filter((arr) => {
            return arr.name
              .toUpperCase()
              .match(new RegExp(fromWhere.toUpperCase()));
  ...
  const [lessThenPrice, setLessThenPrice] = useState(
    _lessThenPrice ? _lessThenPrice : 2000
  );
  ...
  return (
    <div className={styles.category}>
      ...
      <Link
        ...
        className={
          scrollYValue >= 400
            ? styles.showScrollDownCart
            : styles.hideScrollDownCart
        }
      ...
      >
        ...
        <PostNumberPagination
          ...
          setLessThenPrice={setLessThenPrice}
          limit={limit}
          setLimit={setLimit}
          page={page}
          setPage={setPage}
        ...
        {products[0] ? (
              ...
              {products.slice(offset, offset + limit).map((product) => (
                ...
                >
                  ...
                  <Link
                    ...
                    className={styles.productImgName}
                    state={{
                      limit: limit,
                      page: page,
                      products: products,
          ...
          // Loading (Spiner)
          <div
            className={`${styles.spiner} animate-spin inline-block w-40 h-40 border-[20px] border-current border-t-transparent text-zinc-200 rounded-full`}
            role='status'
            aria-label='loading'
          >
