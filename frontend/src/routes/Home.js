...
export default function Home() {
    ...
    useEffect(() => {
        let getNotes = async () => {
            let response = await fetch('https://vyckd353.pythonanywhere.com/api/products/')  // Backend Site
                ...
                {products.map((product) => (
                    <div key={product.id} className={scrollYValue > 790 ? `${styles.product} ${styles.show}` : `${styles.product} ${styles.hidden}`}>
                        ...
                        <div className="flex text-gray-400 flex-col sm:flex-row">
                            <h3>${product.price}&nbsp;</h3>
                            <h3 className="text-red-500 line-through">{product.prevPrice ? '$' + product.prevPrice : ''}</h3>
                        </div>
                        {/* ADD TO CART Button */}
