...
export const AuthProvider = ({ children }) => {
    ...
    const loginUser = async (e) => {
        e.preventDefault()
        let response = await fetch('https://vyckd353.pythonanywhere.com/api/token/', {  // Backend Site
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ 'username': e.target.username.value, 'password': e.target.password.value }) // the value of input name (e.target.password)
        })
        ...
        if (response.status === 200) {
            ...
            let getCartList = async () => {
                let response = await fetch('https://vyckd353.pythonanywhere.com/api/cartlist/') // Backend Site
                let data = await response.json()
                data = await data.filter((arr) => arr.username === e.target.username.value) // Only datas for logged-in-User
                if (data[0]) {
    ...
    const updateToken = async () => {
        if (authTokens) {
            let response = await fetch('https://vyckd353.pythonanywhere.com/api/token/refresh/', {  // Backend Site
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const createUser = async (e) => {
        ...
        if (password == password2 && username && password) {
            let response = await fetch('https://vyckd353.pythonanywhere.com/api/createuser/', { // Backend Site
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
